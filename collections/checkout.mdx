---
title: "Embed KulmiPay Checkout in Your App"
description: "Create a hosted checkout session with your KulmiPay public key and embed or redirect customers to a branded payment page. Supports M-Pesa, cards, and more."
---

KulmiPay Checkout lets you create a payment session on your server and display a pre-built, branded checkout UI in your app — without building a payment form from scratch. You initialise the session with your public key and the amount, then embed the checkout in your page or redirect the customer to it. KulmiPay handles card tokenisation, M-Pesa prompts, and payment state for you.
...
| `unique_api_ref` | boolean | No | When `true`, if a checkout already exists for this `api_ref`, KulmiPay returns the existing session instead of creating a new one. Use this for idempotency. |
...
You can redirect the customer to the `url` from the response, or embed the checkout inside your app using an iframe or KulmiPay's JavaScript widget.
