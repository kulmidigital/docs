...
Send a `POST` request to `/api/v1/payment/collection/pay-requests/` to create a payment link programmatically. Kulmi Pay sends an email to the customer with the payment URL and returns the link details in the response.
...
The `checkout.url` in the response is the URL you can share with your customer. Kulmi Pay also emails this link automatically to the `email` address you provided.
