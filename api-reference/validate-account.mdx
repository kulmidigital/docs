...
A successful request returns HTTP `200` with the account name and validation status. The request is synchronous — KulmiPay queries the provider in real time and waits for a response before returning.
