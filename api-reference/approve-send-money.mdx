...
sidebarTitle: "Approve send money"
description: "Approve a Send Money payment file in PREVIEW-AND-APPROVE state to release it for processing. Accepts file_id or tracking_id from the initiation response."
---

After you initiate a Send Money request with `requires_approval` set to `"YES"` (or omitted), the file waits in the `PREVIEW-AND-APPROVE` state. Calling this endpoint confirms the transactions and releases the file for processing. Kulmi Pay then debits your wallet and routes each payment through the selected provider.
...
<ResponseField name="status" type="string">
  The updated file status. Moves into balance checking and processing after successful approval.
</ResponseField>
...
<ResponseField name="transactions" type="array">
  ...
  <Expandable title="Transaction object fields">
    ...
    <ResponseField name="status" type="string">
      Current transaction status, such as `Pending`, `Payment initiated`, `Processing results`, `Successful`, or `Unsuccessful`.
    </ResponseField>
