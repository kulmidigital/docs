...
description: "List all wallets on your KulmiPay account and create new working wallets to separate funds by purpose, currency, or disbursement use case."
---

Use these endpoints to retrieve your KulmiPay wallets and create new working wallets. Settlement wallets are provisioned automatically when you activate a currency — you only need the `POST` endpoint to create additional working wallets for fund separation or specific disbursement flows.
...
Creates a new working wallet on your account. You cannot create `SETTLEMENT` wallets — those are managed by KulmiPay automatically.
