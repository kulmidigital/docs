---
title: "Kulmi Pay Docs"
description: "Kulmi Pay lets you collect payments and disburse money via M-Pesa, cards, bank transfers, and more. Integrate in minutes with a single REST API."
---

Kulmi Pay is a payment platform built for African businesses. With a single API integration, you can accept payments through M-Pesa, Visa/Mastercard, Apple Pay, Google Pay, and PesaLink bank transfers. Today, live money movement support is centered on Kenya, with broader regional coverage planned over time. Whether you're building a checkout flow, automating payroll, or running recurring billing, Kulmi Pay provides the infrastructure to do it reliably.

<CardGroup cols={2}>
  <Card title="Quick Start" icon="rocket" href="/quickstart">
    Make your first API call and receive a payment in under 10 minutes.
  </Card>
  <Card title="Authentication" icon="key" href="/authentication">
    Learn how to generate API tokens and authenticate every request.
  </Card>
  <Card title="Collections" icon="arrow-down-to-line" href="/collections/overview">
    Accept M-Pesa, card, and bank payments from your customers.
  </Card>
  <Card title="Disbursements" icon="arrow-up-from-line" href="/disbursements/overview">
    Send money to mobile wallets and bank accounts in bulk or one at a time.
  </Card>
  <Card title="Webhooks" icon="bell" href="/webhooks/overview">
    Receive real-time notifications when payments complete or fail.
  </Card>
  <Card title="API Reference" icon="code" href="/api-reference/collect-payment">
    Full reference for every Kulmi Pay endpoint with request and response examples.
  </Card>
</CardGroup>

## How it works

<Steps>
  <Step title="Create your account">
    Sign up at [app.kulmipay.com](https://app.kulmipay.com) and complete business verification to unlock live payments.
  </Step>
  <Step title="Get your API credentials">
    From your dashboard, navigate to **API Applications** to create an application and obtain your Client ID, Client Secret, and Public Key.
  </Step>
  <Step title="Test in the sandbox">
    Use `https://sandbox.kulmipay.com` with sandbox credentials to simulate collections and disbursements without moving real money.
  </Step>
  <Step title="Go live">
    Switch your base URL to `https://app.kulmipay.com` and your live credentials to start processing real transactions.
  </Step>
</Steps>

## Key capabilities

<CardGroup cols={3}>
  <Card title="M-Pesa Payments" icon="mobile" href="/collections/mpesa-stk-push">
    Trigger STK push prompts directly to your customers' phones.
  </Card>
  <Card title="Card Payments" icon="credit-card" href="/collections/card-payments">
    Accept Visa, Mastercard, Apple Pay, and Google Pay globally.
  </Card>
  <Card title="Bulk Disbursements" icon="money-bill-transfer" href="/disbursements/send-money">
    Pay hundreds of recipients in a single API call with full status tracking.
  </Card>
  <Card title="Multi-Currency Wallets" icon="wallet" href="/wallets/overview">
    Hold balances in KES, USD, EUR, and GBP with built-in forex exchange.
  </Card>
  <Card title="Subscriptions" icon="rotate" href="/subscriptions">
    Automate recurring billing with flexible plans and customer management.
  </Card>
  <Card title="Payment Links" icon="link" href="/collections/payment-links">
    Share a URL to collect payments — no coding required.
  </Card>
</CardGroup>
