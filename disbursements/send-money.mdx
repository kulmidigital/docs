---
title: "Send Money via M-Pesa, PesaLink, and Bank Transfer"
...
To send money, you initiate a disbursement request with one or more recipient transactions. KulmiPay validates your request, creates a payment file, and either waits for your approval or processes the payment immediately — depending on how you configure `requires_approval`.

<Steps>
  <Step title="Initiate the disbursement">
    Call `POST /api/v1/send-money/initiation/` with your currency, provider, and a list of transactions. KulmiPay creates a payment file and returns a `file_id` and `tracking_id`.
  </Step>
  <Step title="Review and approve">
    If `requires_approval` is `"YES"` (the default), call `POST /api/v1/send-money/approve/` with the `file_id` to release the payment. If `requires_approval` is `"NO"`, KulmiPay approves and queues the payment automatically — no second call needed.
  </Step>
...
To protect against duplicate payments (for example, if a network timeout causes you to retry a request), include a unique `idempotency_key` per transaction. If KulmiPay receives a second request with the same key for your business, it will reject it with an error rather than processing the payment twice.
